...
public class ExamTakenDao extends JdbcDaoSupport {
    ...
    public boolean insert(ExamTaken examTaken) {
        String sql = "INSERT INTO exam_taken1 (exam_id, student_id, pedagogue_id, holding_date, result) values (?, ?, ?, ?, ?)";
    ...
    public ExamTaken getExamTakenByStudent(long examId, long studentId) {
        String sql = "SELECT * FROM exam_taken1 WHERE exam_id = ? AND student_id = ?";
    ...
    public List<ExamTaken> examsTakenByStudent(long studentId) {
        String sql = "SELECT c.description AS course_description, e.header, e.description, et.* FROM exam_taken1 et " +
    ...
    public ExamTaken lastExamTakenByStudent(long studentId) {
        String sql = "SELECT TOP 1 c.description AS course_description, e.header, e.description, et.* FROM exam_taken1 et " +
                "LEFT JOIN exam e ON e.id = et.exam_id " +
                "LEFT JOIN course c ON c.id = e.course_id " +
                "WHERE student_id = ? " +
                "ORDER BY et.id DESC";
    ...
    public List<ExamTaken> examResults(long examId) {
        String sql = "SELECT et.*, s.first_name, s.last_name FROM exam_taken1 et " +
